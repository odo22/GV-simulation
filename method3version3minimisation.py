...
# Save simulation data in a text file made out of columns delimited by a space
    ...
    profile(n_list, N)
    funtomini(RNAmeans, datamini, lamb)
    plt.show()

def funtomini(RNAmeans, datamini, lamb):
    f=[]
    RNAm = RNAmeans.mean(0)
    print(RNAm.shape)
    for i in range(lamb.size-1):
        f[i] = RNAm[i]-datamini[i,1]
    #residuals = np.sum(f, axis=0)
    print(residuals)
    return residuals
